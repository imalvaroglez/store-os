...
4. Recibo imprimible/PDF por Order: `window.print()` + CSS `@media print`,
   tamaño carta/A4, plantilla fija con la marca de Olivia (referencia Canva de
   Fer), campos fiscal-ready con defaults invisibles.
5. Folio consecutivo por tienda (`receiptSeq`, p. ej. `REC-0001`).
...
export type Order = {
  ...
  // Fiscal-ready (defaults invisibles; ver tabla abajo)
  paymentMethod?: PaymentMethod;   // default "undetermined"
  cfdiUse?: string;               // default "G03"
  unitCode?: string;              // default "H87" (pieza)
  customerRfc?: string;           // default: vacío
  // Recibo
  receiptFolio?: string;          // "REC-0001"; asignado al primer emitido
...
- Campos fiscal-ready capturados aquí, con defaults que no estorban: forma de
  pago (default "Por definir"), uso CFDI (G03), unidad (H87/pieza), RFC de la
  clienta (vacío, opcional). Régimen fiscal de la tienda: campo opcional en
  `Store`/configuración de tienda, no en el form.
...
- Nueva pantalla de solo lectura `ReceiptScreen` (ruta tipo
  `/venta/:orderId/recibo`) alcanzada con "Ver recibo" desde `OrderCard`.
- Es una **proyección de solo lectura** de datos existentes (como el catálogo
  público): no hay colección nueva; solo se escriben `receiptFolio` /
  `receiptIssuedAt` en el Order la primera vez que se emite.
- Contenido: encabezado con marca de Olivia (logo/colores de la tienda),
  ...
  importe), total, anticipo, saldo, forma de pago, notas/promisedDate cuando
  existan, pie con datos de contacto (WhatsApp de la tienda).
- Plantilla fija reconstruida en HTML/CSS a partir de la referencia Canva de
  ...
  Canva en runtime). Si la referencia no llegó al momento de implementar, se
  usa una plantilla limpia con los tokens de tema de la tienda y un comentario
  `ponytail:` marcando el ajuste pendiente del diseño exacto.
...
- Folio: al primer emitido, `receiptFolio = "REC-" + pad4(store.receiptSeq + 1)`
  e incrementar `receiptSeq` en `Store`. Concurrencia best-effort igual que el
  claim de slug (transaction en cloud; en demo no aplica). Si el incremento
  falla, el recibo se muestra sin folio y se reintenta — nunca se bloquea la
  venta.
...
- Facturación fiscal / CFDI / timbrado PAC (pospuesto; cobra por comprobante y
  rompe cero-costos). Los campos fiscal-ready existen para no rediseñar después.
...
- Migración = escrituras puntuales una sola vez por documento (≈ #Orders
  existentes, cuota muy por debajo de 20K/día); folio = 2 escrituras por
  primera emisión (Order + Store).
- Sin telemetría; sin PII nueva más allá de RFC opcional capturado por Fer
  (vive en el mismo plano de datos que `Customer`, sujeto a las mismas reglas
  G-P01–G-P08).
...
4. El folio es consecutivo por tienda (`REC-0001`, `REC-0002`, …) y se asigna
   una sola vez.
5. `npm run typecheck && npm run test && npm run build` verdes; helpers de
   total/balance y migración con test pequeño; e2e smoke existente no regresa.
6. UI 100% español, solo design system, mobile-first.

## previewChecks

- path: `/` (app demo) → selector: `[data-testid="nav-ventas"]` o texto
  "Ventas" → texto: pedido existente visible (migrado a items).
  *Alternativa mínima:* abrir un Order y verificar `OrderCard` renderiza el
  total de varias líneas.
- path: `/venta/:orderId/recibo` (o ruta interna equivalente alcanzada con
  "Ver recibo") → selector: `[data-testid="receipt"]` → texto: "Recibo" +
  folio `REC-` + total.
- selector: `[data-testid="receipt-print"]` → texto: "Imprimir / Guardar PDF".
- path: form de nueva venta → selector: `[data-testid="order-form"]` → texto:
  "Agregar producto" (línea adicional) y "Saldo".

## Notas de implementación

- `parseAmount` (`src/lib/money.ts`) para toda coerción numérica; nunca `NaN`
  al estado.
- La pantalla de recibo usa tokens de tema, no colores hardcodeados (regla de
  comportamiento sin sorpresas); el CSS `@media print` sí puede fijar medidas
  en mm porque es papel, no pantalla.
- `ponytail:`: el folio por transaction best-effort, no un contador atómico
  global — una tienda con una sola operadora no tiene colisiones reales.
