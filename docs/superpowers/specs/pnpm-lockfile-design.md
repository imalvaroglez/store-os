...
```json
[
  { "path": "/", "selector": "text=/¿Quién opera hoy\\?|Iniciar sesión/", "text": "Store OS" },
  { "path": "/catalogo/olivia", "selector": "body", "text": "Olivia" }
]
```

(Verificación de humo de que la app sigue arrancando tras el cambio de
toolchain; el catálogo público cubre la ruta anónima.)
