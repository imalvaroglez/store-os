...
- **Correo verificado como prerrequisito.** El token incluye `email_verified`; comparar sólo el email del token no demuestra propiedad del correo. **Guard explícito antes de `ensureUserDoc`/reconciliación en los CUATRO entry points** (email, Google, email-link, y montaje de `AuthProvider` con sesión previa): si `user.emailVerified` es falso, no se crea/actualiza `users/{uid}`, no se muestra el panel privado y no se reconcilia nada. En email/password: tras registrarse se envía verificación (`sendEmailVerification`); al siguiente login con `email_verified == true`, `ensureUserDoc` crea o completa el perfil (incluye `emailVerified: true` y el backfill de `emailNormalized` — perfiles legacy se completan en ese momento). Google y email-link entregan correo ya verificado. La reconciliación corre "en cada inicio de sesión **con correo verificado**" — nunca antes.
...
- **Rules (`npm run test:rules`):** invitado lista solo su tienda; invitee-update aceptado solo con el diff exacto; intento de escalar ownerUid o tocar otro campo rechazado; owner-path sin regresión; spoofing de `email`/`emailNormalized`/`emailVerified` rechazado en `users.create` y self-update; token sin `email_verified` no crea perfil ni reclama invitaciones; update admin cross-user cambia `role` sin alterar identidad; Google/email-link y password verificado funcionan.
- **Unit/integración (cliente, vitest):** el flujo NO verificado — el guard bloquea `ensureUserDoc`/reconciliación en los cuatro entry points; `findUidByEmail` (ambos intentos) no devuelve perfiles sin verificar (no aparecen en búsqueda).
