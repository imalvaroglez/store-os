...
- **GAP (estado actual):** HOY `isSuperAdmin()` hace short-circuit al inicio de `isMember`/`isOwner` (`firestore.rules:14,19`) y el cliente lee la vista-dios (`firestoreData.ts:33,82-84,123-142`), de modo que `super_admin` lee TODA la PII de TODAS las tiendas. **Esta garantía NO se cumple hoy.** La vista-dios actual es un **GAP de migración, no una facultad aceptada ni un riesgo contratado**. Alcanzar G-P02 exige: (a) crear `adminStores` canónico con la allow-list declarada, (b) acotar el short-circuit para que la lectura de `stores` y datos privados requiera `memberUids`, (c) que `super_admin` lea control sólo vía `adminStores`, y (d) mantener `adminStores` sincronizado en escritura conjunta. Trabajo de implementación. Ver §9.
...
- **GAP (estado actual):** HOY `@vercel/analytics` y `@vercel/speed-insights` están montados globalmente en `src/main.tsx:3-4` (en TODAS las rutas). **Esta garantía NO se cumple hoy.** V1 los retira por completo (ver Espec 2 §5.2-5.3 y la memoria de decisión). Hasta el retiro + la prueba de egress, G-P08 es norma, no realidad.
...
- **GAP-G-P02 (el mayor):** `isSuperAdmin()` short-circuit en `isMember`/`isOwner` (`firestore.rules:14,19`) + cliente vista-dios (`firestoreData.ts:33,82-84,123-142`) hacen que `super_admin` lea TODA la PII de TODAS las tiendas. **La vista-dios es un GAP de migración, no una facultad aceptada ni contratada.** Alcanzar G-P02 exige (a) crear `adminStores` como documento **canónico** del plano de control con la allow-list declarada, (b) acotar el short-circuit para que la lectura de `stores` y datos privados requiera `memberUids`, (c) que `super_admin` lea control sólo vía `adminStores`, y (d) sincronizar `adminStores`↔`stores` en escritura conjunta — porque Firestore **no puede** ocultar campos en una lectura de `stores`. Hasta entonces, G-P02 es norma, no realidad.
