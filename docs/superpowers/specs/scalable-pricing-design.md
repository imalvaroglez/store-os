...
- **Propagación de error en la proyección (requisito):** hoy `updateStore` silencia el error de `projectPublicForStore` (`src/app/StoreProvider.tsx:276`) — puede persistir el default privado, aparentar éxito y dejar el precio público anterior. Esta entrega lo corrige: si la proyección falla, el error **se muestra** (sin éxito falso; toast de error). **Sin garantía de atomicidad ni de que la proyección anterior quede intacta:** `projectPublicForStore` (`src/app/firebase/firestoreData.ts:444`) hace escrituras independientes y puede fallar parcialmente — hacerlo atómico sería sobreingeniería. **Dos rutas de reparación, con costos distintos:**
  - **Volver a guardar** (`updateStore`): idempotente, 2 writes privados (`stores` + `adminStores`) + proyección completa, por intento.
  - **"Republicar catálogo"** (`republishCatalog`, `src/app/StoreProvider.tsx:341`): **sin writes privados** — ejecuta `claimSlug` (≥1 lectura transaccional; hasta 1 write si el slug aún no está reclamado) + la proyección pública completa.
  **Tests explícitos, uno por ruta:** (1) re-guardado completo — primer guardado falla en la proyección (error visible, sin toast de éxito) → segundo guardado **tiene éxito** y se afirma que ejecutó 2 writes privados + la proyección completa; (2) `republishCatalog` — repara la proyección sin escribir `stores`/`adminStores`.
...
- **Proyecciones stale / fallidas:** la migración persiste por adapter y república una vez por tienda; cada guardado de tiers dispara la proyección completa (con error visible si falla — sin éxito falso). Una proyección parcialmente escrita o desfasada se repara preferentemente con `republishCatalog` (claimSlug: ≥1 lectura transaccional + hasta 1 write si falta el slug + proyección completa; sin writes privados); volver a guardar cuesta 2 writes privados + proyección por intento.
