...
- **Cloud:** corre en cliente al cargar estado (`StoreProvider`), igual que hoy; no se requiere script de servidor. El guardado de tiers editados escribe `stores/{id}.priceTiers` (1 write por edición de ajustes, dentro de cuota).
...
- **Proyecciones stale:** la migración republica cada tienda afectada una vez; tras editar tiers/labels/default, el botón "Republicar catálogo" existente cubre los cambios manuales posteriores.
