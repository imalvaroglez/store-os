...
- **Propagación de error en la proyección (requisito):** hoy `updateStore` silencia el error de `projectPublicForStore` (`src/app/StoreProvider.tsx:276`) — puede persistir el default privado, aparentar éxito y dejar el precio público anterior. Esta entrega lo corrige: si la proyección falla, el error **se muestra** (sin éxito falso; toast de error) y se permite reparar con **`republishCatalog`** (el botón "Republicar catálogo" existente). **Sin garantía de atomicidad ni de que la proyección anterior quede intacta:** `projectPublicForStore` (`src/app/firebase/firestoreData.ts:444`) hace escrituras independientes y puede fallar parcialmente — hacerlo atómico sería sobreingeniería. El reintento es **idempotente pero se contabiliza**: `updateStore` re-persiste ambos planos en cada llamada, así que cada intento cuesta 2 writes privados + proyección completa (más cualquier escritura parcial del intento fallido). Con test que falle la proyección y afirme: error visible, sin toast de éxito, y reparación vía `republishCatalog`.
