...
Return one JSON object with `status`, `summary`, nonempty `evidence`, `findings`, and `reviewer`. `reviewer` contains the unique run ID assigned by the parent, profile `store-os-reviewer`, and the exact lens. Each finding needs `id`, `blocking`, `claim`, and `evidence`.
